...
各候補には一次情報へのリンク、著者、公開日、種別、掲載先、識別子、選定理由、関連性スコア、定量指標、検索テーマを記載する。
...
Crossrefでは、次の会議の新規登録とメタデータ更新を監視する。
...
HRIとRO-MANは会議全体を候補にする。

IVA、CUI、ICMI、INTERSPEECH、SemDialは対象範囲が広いため、社会ロボット、対話、社会的相互作用、マルチモーダル、人格、感情などの焦点語に一致した論文だけを候補にする。

Crossref検索は公開日ではなく更新日を使う。

出版社が過去の公開日を登録した場合や、登録後にメタデータを補完した場合も拾うためである。

候補は最初に、タイトルと抄録のアンカー語・トピック語、対象学会、独立した収集元の数から関連性を評価する。

アンカー語または対象学会への一致があり、関連性スコアが30点以上の候補だけをレビュー対象にする。

語彙と重みは `sources/queries.yaml` の `relevance` で管理する。

外部APIへの照会量を抑えるため、RedditとXの指標は関連性スコアによる上位20件へ付与する。
...
定量指標を満たさない候補のために、レビューキュー10件のうち3件を新着発見枠として確保する。

各枠では関連性スコアを最優先し、その後に対数補正した被引用数、影響力の高い引用、Reddit、Xの注目度スコアを評価する。

検索語の一致数は最後の同点判定にだけ使用する。

これにより、被引用数は多いが対象外の文献がレビューキューを占有することを防ぎつつ、公開直後で被引用数がない研究を救済する。
